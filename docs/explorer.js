...
import { applyPageMetadata, encodersByName } from "./encoders.js";
...
nav.innerHTML = encoderEntries.map(({ name, label, shortLabel }) =>
  `<a href="#${name}" data-encoder="${name}" aria-label="${stripTags(label)}"><span class="full-label">${label}</span><span class="short-label">${shortLabel}</span></a>`
).join(" ");
...
function render() {
  ...
  renderMath(`\\alpha=${currentLatex}`, currentNode, { displayMode: true });
  ...
  for (const row of rows.children) {
    ...
    row.hidden = false;
    row.setAttribute("aria-label", `alpha ${index}`);
    renderOption(row, index, childLatex, "value");
  }
...
function renderOption(row, index, latex, state) {
  const href = compactHashAfterMove(index);
  const renderKey = `${state}:${index}:${href}:${latex}`;
...
function highlightEncoder() {
  applyPageMetadata(currentEncoder.explorerTitle, currentEncoder.favicon);
