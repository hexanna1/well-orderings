...
function createRawDecoder(primeIndex, rawCodeCache = null) {
  ...
  function rawDecode(rawCode, factors = null) {
    ...
    const rawTerms = (factors ?? primeIndex.factorize(rawCode)).map(([prime, coeff]) => {
      const sourceIndex = primeIndex.primeIndex(prime);
      const level = sourceIndex % 2 === 0 ? 1 : 0;
      const childCode = level === 0 ? (sourceIndex + 1) / 2 : sourceIndex / 2;
      return term(level, rawDecode(childCode), coeff);
    });
    const value = makeOrdinal(rawTerms, false);
...
export function createAdmissibilityScanner(primeIndex) {
  ...
  return Object.freeze({
    rawDecode,
    isAdmissible: (candidate) => isAdmissible(0, candidate),
  });
}
...
export function createEncoder(primeIndex, admissibleData) {
  ...
  function ordinal(n) {
    ...
    const terms = primeIndex.factorize(n).map(([prime, coeff]) => {
      ...
      const arg = admissibleUnrank(
        ...
        admissibleData,
        rawDecode,
        (candidate) => isAdmissible(0, candidate),
      );
      return term(0, arg, coeff);
    });
  ...
  function rawCode(value) {
    ...
    const cached = rawCodeCache.get(ordinalValue);
    if (cached !== undefined) {
      return cached;
    }

    let n = 1;
    for (const {principal: principalValue, coeff} of ordinalValue.terms) {
      const childCode = rawCode(principalValue.arg);
      if (childCode === null) {
        n = null;
        break;
      }
      const sourceIndex = 2 * childCode - 1 + principalValue.level;
      n = boundedPrimePowerProduct(n, sourceIndex, coeff, primeIndex);
      if (n === null) {
        break;
      }
    }
    rawCodeCache.set(ordinalValue, n);
    return n;
  }
  ...
  function natural(value) {
    ...
    if (
      !isLegal(ordinalValue) ||
      ordinalValue.terms.some(({principal: principalValue}) => principalValue.level !== 0)
    ) {
      naturalCache.set(ordinalValue, null);
      return null;
    }
...
function usesLevel(ordinal, level) {
  const value = asOrdinal(ordinal);
  return cachedByNumber(usesLevelCache, value, level, () =>
    value.terms.some(
      (item) =>
...
function startsAboveLevel(ordinal, level) {
  ...
  return value.terms.length > 0 &&
    value.terms[0].principal.level > level;
}
...
function isEpsilonNumber(ordinal) {
  ...
  return item.coeff === 1 &&
    startsAboveLevel(item.principal.arg, item.principal.level);
}
...
function fsAt(alpha, n) {
  ...
  return cachedByNumber(fsAtCache, value, n, () => {
...
function support(level, ordinal) {
  validateLevel(level);
  const value = asOrdinal(ordinal);
  return cachedByNumber(supportCache, value, level, () => {
    const result = new Map();
...
function isAdmissible(level, arg) {
  ...
  return cachedByNumber(admissibleCache, value, level, () =>
    isLegal(value) &&
...
function isLegal(ordinal) {
  const value = asOrdinal(ordinal);
  const cached = legalCache.get(value);
  if (cached !== undefined) {
    return cached;
  }
