import {
  boundedMultiplyPower,
  boundedPrimePowerProduct,
  cacheIntegerValue,
} from "../natural_number_encoding.js";
...
const ordinalCompareCache = new WeakMap();
const matrixCompareCache = new WeakMap();
const predecessorCache = new WeakMap();
const limitCache = new WeakMap();
const fsAtCache = new WeakMap();
...
function weakPairGet(cache, left, right) {
  return cache.get(left)?.get(right);
}

function weakPairSet(cache, left, right, value) {
  let values = cache.get(left);
  if (values === undefined) {
    values = new WeakMap();
    cache.set(left, values);
  }
  values.set(right, value);
}

function cachedByNumber(cache, owner, number, compute) {
  let values = cache.get(owner);
  if (values === undefined) {
    values = new Map();
    cache.set(owner, values);
  } else if (values.has(number)) {
...
export function createEncoder(primeIndex) {
  const ordinalCache = new Map();
  const naturalCache = new WeakMap();

  function splitColumnFactorIndex(sourceIndex) {
    let positionCode = 1;
    let topTermIndex = 1;
    for (const [sourcePrime, exponent] of primeIndex.factorize(sourceIndex)) {
      const sourcePrimeIndex = primeIndex.primeIndex(sourcePrime);
      const childIndex = sourcePrimeIndex % 2 === 0
        ? sourcePrimeIndex / 2
        : (sourcePrimeIndex + 1) / 2;
      const childPrime = primeIndex.primeAtIndex(childIndex);
      if (sourcePrimeIndex % 2 === 0) {
        topTermIndex *= childPrime ** exponent;
      } else {
        positionCode *= childPrime ** exponent;
      }
    }
    return [positionCode, topTermIndex];
  }

  function ordinal(n) {
    assertSafePositiveInteger(n, "n");
    const cached = ordinalCache.get(n);
    if (cached !== undefined) {
      return cached;
    }

    const terms = primeIndex.factorize(n).map(([prime, coefficient]) => {
      const entryCodes = new Map();
      for (const [sourcePrime, exponent] of primeIndex.factorize(primeIndex.primeIndex(prime))) {
        const [positionCode, topTermIndex] = splitColumnFactorIndex(
          primeIndex.primeIndex(sourcePrime),
        );
        const topTermPrime = primeIndex.primeAtIndex(topTermIndex);
        entryCodes.set(
          positionCode,
          (entryCodes.get(positionCode) ?? 1) * topTermPrime ** exponent,
        );
      }
      const entries = Array.from(
        entryCodes,
        ([positionCode, valueCode]) => [
          ordinal(positionCode),
          ordinal(valueCode),
        ],
      );
      return uncheckedTerm(repairFixedPointTail(matrix(entries)), coefficient);
    });
    const value = fromTerms(terms);
    return cacheIntegerValue(ordinalCache, n, value);
  }

  function encodeIndexPart(code, even) {
    if (code === null) {
      return null;
    }
    let encoded = 1;
    for (const [prime, exponent] of primeIndex.factorize(code)) {
      const childIndex = primeIndex.primeIndex(prime);
      const sourceIndex = 2 * childIndex - (even ? 0 : 1);
      encoded = boundedPrimePowerProduct(encoded, sourceIndex, exponent, primeIndex);
      if (encoded === null) {
        return null;
      }
    }
  ...
  function encodeColumnFactorIndex(positionCode, topTermIndex) {
    const oddPart = encodeIndexPart(positionCode, false);
    const evenPart = encodeIndexPart(topTermIndex, true);
    if (oddPart === null || evenPart === null) {
      return null;
    }
    return boundedMultiplyPower(oddPart, evenPart, 1, primeIndex.maximum);
  }

  function encodeValueCodeAtPosition(code, positionCode) {
    if (code === null || positionCode === null) {
      return null;
    }
    let encoded = 1;
    for (const [prime, exponent] of primeIndex.factorize(code)) {
      const topTermIndex = primeIndex.primeIndex(prime);
      const sourceIndex = encodeColumnFactorIndex(positionCode, topTermIndex);
      encoded = boundedPrimePowerProduct(encoded, sourceIndex, exponent, primeIndex);
      if (encoded === null) {
        return null;
      }
  ...
  function indexFromMatrix(matrixValue) {
    const value = matrix(asMatrix(matrixValue).entries);
    if (!isPreferredMatrix(value)) {
      return null;
    }

    let index = 1;
    for (const [position, entryOrdinal] of undoFixedPointTailRepair(value).entries) {
      const encoded = encodeValueCodeAtPosition(
        natural(entryOrdinal),
        natural(position),
      );
      if (encoded === null) {
        return null;
      }
      index = boundedMultiplyPower(index, encoded, 1, primeIndex.maximum);
      if (index === null) {
        return null;
      }
    }
  ...
  function natural(value) {
    const input = asOrdinal(value);
    const cached = naturalCache.get(input);
    if (cached !== undefined) {
      return cached;
    }
    const ordinalValue = canonicalizeOrdinal(input);

    let n = 1;
    for (const {matrix: matrixValue, coefficient} of ordinalValue.terms) {
      n = boundedPrimePowerProduct(
        n,
        indexFromMatrix(matrixValue),
        coefficient,
        primeIndex,
      );
      if (n === null) {
    ...
    naturalCache.set(input, n);
    return n;
  }

  return Object.freeze({ordinal, natural});
}
...
function withoutFiniteTail(ordinal) {
  const value = asOrdinal(ordinal);
  const terms = value.terms.slice();
  if (terms.length === 0 || !isMatrixOne(terms[terms.length - 1].matrix)) {
    return {core: value, tail: 0};
  }
  const tail = terms.pop().coefficient;
  return {core: terms.length === 0 ? zero : fromTerms(terms), tail};
}

function withFiniteTail(core, tail) {
  assertSafeNonnegativeInteger(tail, "tail");
  if (tail === 0) {
    return core;
  }
  return fromTerms([...core.terms, uncheckedTerm(matrixOne, tail)]);
}

function repairFixedPointTail(matrixValue) {
  const value = matrix(asMatrix(matrixValue).entries);
  const active = entryValue(value.entries, zero);
  const {core, tail} = withoutFiniteTail(active);
  const coreMatrix = withEntry(value.entries, zero, core);
  if (!isPreferredMatrix(coreMatrix)) {
    const repaired = withEntry(value.entries, zero, withFiniteTail(core, tail + 1));
    if (!isPreferredMatrix(repaired)) {
      throw new RangeError("could not repair raw matrix descriptor into preferred normal form");
    }
    return repaired;
  }
  if (!isPreferredMatrix(value)) {
    throw new RangeError("raw matrix descriptor is not preferred and has no active finite-tail repair");
  }
  return value;
}

function undoFixedPointTailRepair(matrixValue) {
  const value = matrix(asMatrix(matrixValue).entries);
  const active = entryValue(value.entries, zero);
  if (isZero(active)) {
    return value;
  ...
  const coreMatrix = withEntry(value.entries, zero, core);
  return isPreferredMatrix(coreMatrix)
    ? value
    : withEntry(value.entries, zero, withFiniteTail(core, tail - 1));
}
...
function compareLesserRemainder(lesserEntries, differingPosition, greaterOrdinal) {
  for (let index = 0; index < lesserEntries.length; index += 1) {
    const [position, value] = lesserEntries[index];
    ...
    return index + 1 < lesserEntries.length ? 1 : 0;
  }
  return -1;
}
...
function compareMatrix(a, b) {
  ...
  const cached = weakPairGet(matrixCompareCache, left, right);
  if (cached !== undefined) {
    return cached;
  }
  ...
  while (leftIndex < left.entries.length || rightIndex < right.entries.length) {
    ...
    let leftValue;
    let rightValue;
    if (rightIndex >= right.entries.length) {
      [position, leftValue] = left.entries[leftIndex];
      rightValue = zero;
      leftIndex += 1;
    } else if (leftIndex >= left.entries.length) {
      [position, rightValue] = right.entries[rightIndex];
      leftValue = zero;
      rightIndex += 1;
    } else {
      const [leftPosition, currentLeftValue] = left.entries[leftIndex];
      const [rightPosition, currentRightValue] = right.entries[rightIndex];
      const positionCmp = compareOrdinal(leftPosition, rightPosition);
      if (positionCmp > 0) {
        position = leftPosition;
        leftValue = currentLeftValue;
        rightValue = zero;
        leftIndex += 1;
      } else if (positionCmp < 0) {
  ...
  weakPairSet(matrixCompareCache, left, right, result);
  weakPairSet(matrixCompareCache, right, left, -result);
  return result;
}
...
function compareOrdinal(a, b) {
  ...
  const cached = weakPairGet(ordinalCompareCache, left, right);
  if (cached !== undefined) {
    return cached;
  }
  if (isZero(left)) {
    weakPairSet(ordinalCompareCache, left, right, -1);
    weakPairSet(ordinalCompareCache, right, left, 1);
    return -1;
  }
  if (isZero(right)) {
    weakPairSet(ordinalCompareCache, left, right, 1);
  ...
  weakPairSet(ordinalCompareCache, left, right, result);
  weakPairSet(ordinalCompareCache, right, left, -result);
  return result;
}
...
export function predecessor(ordinal) {
  const value = asOrdinal(ordinal);
  const cached = predecessorCache.get(value);
  if (cached !== undefined) {
    return cached;
  }
  let result;
  ...
  predecessorCache.set(value, result);
  return result;
}

function isLimit(ordinal) {
  const value = asOrdinal(ordinal);
  const cached = limitCache.get(value);
  if (cached !== undefined) {
    return cached;
  }
  const result = !isZero(value) && predecessor(value) === null;
  limitCache.set(value, result);
  return result;
}
...
function fsAt(alpha, n) {
  ...
  return cachedByNumber(fsAtCache, value, n, () => fsOrdinal(value, n));
}
...
export function children(alpha, buttonCount = 4) {
  ...
  return cachedByNumber(childrenCache, value, buttonCount, () => {
    const result = [];
