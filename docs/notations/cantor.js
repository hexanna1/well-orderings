...
export function createEncoder(primeIndex) {
  ...
  function ordinal(n) {
    ...
    const value = fromTerms(
      primeIndex.factorize(n).map(([prime, coefficient]) => [
        ordinal(primeIndex.primeIndex(prime)),
        coefficient,
      ]),
    );
  ...
  function natural(value) {
    ...
    let n = 1;
    for (const {exponent, coefficient} of ordinalValue.terms) {
      n = boundedPrimePowerProduct(n, natural(exponent), coefficient, primeIndex);
      if (n === null) {
        break;
      }
    }
...
function fsAt(alpha, n) {
  ...
  return cachedByNumber(fsAtCache, alpha, n, () => fsAtLimit(alpha, n));
}
