...
const admissibleCache = new WeakMap();
const supportCache = new WeakMap();
const belowEpsilon0Cache = new WeakMap();
const usesLevelCache = new WeakMap();
...
function createRawDecoder(primeIndex, rawCodeCache = null) {
  const rawOrdinalCache = new Map();

  function unpackSourceIndex(sourceIndex) {
    let levelCode = 1;
    let argCode = 1;
    for (const [prime, exponent] of primeIndex.factorize(sourceIndex)) {
      const index = primeIndex.primeIndex(prime);
      const childPrime = primeIndex.primeAtIndex(
        index % 2 === 0 ? index / 2 : (index + 1) / 2,
      );
      if (index % 2 === 0) {
        argCode *= childPrime ** exponent;
      } else {
        levelCode *= childPrime ** exponent;
      }
    }
    return [levelCode, argCode];
  }

  function rawDecode(rawCode, factors = null) {
    const rawCached = rawOrdinalCache.get(rawCode);
    if (rawCached !== undefined) {
      return rawCached;
    }
    const terms = (factors ?? primeIndex.factorize(rawCode)).map(([prime, coeff]) => {
      const [levelCode, argCode] = unpackSourceIndex(primeIndex.primeIndex(prime));
      return {level: rawDecode(levelCode), arg: rawDecode(argCode), coeff};
    });
    const value = makeOrdinal(terms);
    cacheIntegerValue(rawOrdinalCache, rawCode, value);
    rawCodeCache?.set(value, rawCode);
    return value;
  }
...
export function createAdmissibilityScanner(primeIndex) {
  const rawDecode = createRawDecoder(primeIndex);
  return Object.freeze({
    rawDecode,
    isAdmissible: (candidate) => isAdmissible(zero, candidate),
  });
}

export function createEncoder(primeIndex, admissibleData) {
  const ordinalCache = new Map();
  const naturalCache = new WeakMap();
  const rawCodeCache = new WeakMap();
  const rawDecode = createRawDecoder(primeIndex, rawCodeCache);
  ...
  function ordinal(n) {
    ...
    const terms = primeIndex.factorize(n).map(([prime, coeff]) => {
      const rank = primeIndex.primeIndex(prime);
      const arg = admissibleUnrank(
        rank,
        admissibleData,
        rawDecode,
        (candidate) => isAdmissible(zero, candidate),
      );
      return {level: zero, arg, coeff};
    });
    const value = makeOrdinal(terms, {checkLegal: true});
  ...
  function packSourceIndex(levelCode, argCode) {
    const levelPart = encodeIndexPart(levelCode, false);
    const argPart = encodeIndexPart(argCode, true);
    if (levelPart === null || argPart === null) {
      return null;
    }
    return boundedMultiplyPower(levelPart, argPart, 1, primeIndex.maximum);
  }

  function rawCode(value) {
    assertOrdinal(value);
    const cached = rawCodeCache.get(value);
    if (cached !== undefined) {
      return cached;
    }

    let n = 1;
    for (const {level, arg, coeff} of value.terms) {
      const sourceIndex = packSourceIndex(rawCode(level), rawCode(arg));
      n = boundedPrimePowerProduct(n, sourceIndex, coeff, primeIndex);
      if (n === null) {
        break;
      }
    }
    rawCodeCache.set(value, n);
    return n;
  }

  function natural(value) {
    assertOrdinal(value);
    const cached = naturalCache.get(value);
    if (cached !== undefined) {
      return cached;
    }
    if (!isLegal(value) || value.terms.some(({level}) => !isZero(level))) {
      naturalCache.set(value, null);
      return null;
    }

    let n = 1;
    for (const {arg, coeff} of value.terms) {
      const rank = admissibleRank(
        arg,
        admissibleData,
        rawCode,
        rawDecode,
      ...
      n = boundedPrimePowerProduct(n, rank, coeff, primeIndex);
      if (n === null) {
        break;
      }
    }
    naturalCache.set(value, n);
    return n;
  }
...
function support(level, ordinal) {
  const cached = weakPairGet(supportCache, level, ordinal);
  if (cached) {
    return cached;
  }
  ...
  weakPairSet(supportCache, level, ordinal, result);
  return result;
}
...
function isAdmissible(level, arg) {
  assertOrdinal(level, "level");
  assertOrdinal(arg, "argument");
  const cached = weakPairGet(admissibleCache, level, arg);
  if (cached !== undefined) {
    return cached;
  }
  ...
  weakPairSet(admissibleCache, level, arg, result);
  return result;
}
...
function isLegal(ordinal) {
  assertOrdinal(ordinal);
  const cached = legalCache.get(ordinal);
  if (cached !== undefined) {
    return cached;
  }

  const result = ordinal.terms.every((term) => isLegalPrincipal(term));
  legalCache.set(ordinal, result);
  return result;
}

function usesLevelAtLeast(ordinal, threshold) {
  const cached = weakPairGet(usesLevelCache, ordinal, threshold);
  if (cached !== undefined) {
    return cached;
  }
  ...
  weakPairSet(usesLevelCache, ordinal, threshold, result);
  return result;
}

function isBelowEpsilon0(ordinal) {
  const cached = belowEpsilon0Cache.get(ordinal);
  if (cached !== undefined) {
    return cached;
  }
  const result = !usesLevelAtLeast(ordinal, one);
  belowEpsilon0Cache.set(ordinal, result);
  return result;
}

function startsAboveLevel(ordinal, level) {
  return ordinal.terms.length > 0 &&
    compare(ordinal.terms[0].level, level) > 0;
}
...
function isEpsilonNumber(ordinal) {
  ...
  return term.coeff === 1 &&
    startsAboveLevel(term.arg, term.level);
}
...
function fsAt(alpha, n) {
  ...
  return cachedByNumber(fsAtCache, alpha, n, () => fsAtIndex(alpha, finite(n)));
}
...
export function children(alpha, buttonCount = 4) {
  ...
  return cachedByNumber(childrenCache, alpha, buttonCount, () =>
    Object.freeze(Array.from({ length: buttonCount }, (_, n) => fsAt(alpha, n))),
  );
}
