...
function withoutFiniteTail(ordinal) {
  ...
  if (terms.length === 0 || !isPhiOne(terms[terms.length - 1].phi)) {
...
function argumentNotFixedPointOf(beta, gamma) {
  const {core, tail} = withoutFiniteTail(gamma);
  if (!isFixedPointCore(beta, core.terms)) {
    return gamma;
  }
  return addFinite(core, tail + 1);
}

function rawArgumentFromRepaired(beta, gamma) {
  const value = asOrdinal(gamma);
  if (value.terms.length === 0 || !isPhiOne(value.terms[value.terms.length - 1].phi)) {
    return value;
  }
  const {core, tail} = withoutFiniteTail(value);
  return tail > 0 && isFixedPointCore(asOrdinal(beta), core.terms)
    ? addFinite(core, tail - 1)
    : value;
}
...
export function createEncoder(primeIndex) {
  ...
  function ordinal(n) {
    ...
    const terms = primeIndex.factorize(n).map(([prime, coefficient]) => {
      const source = primeIndex.primeIndex(prime);
      let betaCode = 1;
      let gammaCode = 1;
      for (const [sourcePrime, exponent] of primeIndex.factorize(source)) {
        const sourceIndex = primeIndex.primeIndex(sourcePrime);
        const childIndex = sourceIndex % 2 === 0 ? sourceIndex / 2 : (sourceIndex + 1) / 2;
        const childPrime = primeIndex.primeAtIndex(childIndex);
        if (sourceIndex % 2 === 0) {
          betaCode *= childPrime ** exponent;
        } else {
          gammaCode *= childPrime ** exponent;
        }
      }
      const beta = ordinal(betaCode);
      const gamma = argumentNotFixedPointOf(beta, ordinal(gammaCode));
      return uncheckedTerm(uncheckedPhi(beta, gamma), coefficient);
    });
  ...
  function encodeChildCodeAtArgument(code, argumentIndex) {
    ...
    for (const [prime, exponent] of primeIndex.factorize(code)) {
      ...
      const sourceIndex = 2 * childIndex - 1 + argumentIndex;
  ...
  function indexFromPhi(phiTerm) {
    const gammaCode = natural(rawArgumentFromRepaired(phiTerm.beta, phiTerm.gamma));
    const betaCode = natural(phiTerm.beta);
    const gammaIndex = encodeChildCodeAtArgument(gammaCode, 0);
    const betaIndex = encodeChildCodeAtArgument(betaCode, 1);
    if (gammaIndex === null || betaIndex === null) {
      return null;
    }
    return boundedMultiplyPower(gammaIndex, betaIndex, 1, primeIndex.maximum);
  }

  function natural(value) {
    const ordinalValue = asOrdinal(value);
    ...
    let n = 1;
    for (const {phi: phiTerm, coefficient} of ordinalValue.terms) {
      n = boundedPrimePowerProduct(n, indexFromPhi(phiTerm), coefficient, primeIndex);
      if (n === null) {
        break;
      }
    }
