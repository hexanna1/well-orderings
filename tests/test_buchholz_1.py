...
class Buchholz1MappingTests(unittest.TestCase):
    ...
    def test_buchholz_1_code_roundtrip_on_prefix(self) -> None:
        enc = Buchholz1OrdinalEncoder()
        for n in range(1, 301):
            self.assertEqual(enc.buchholz_1_encode(enc.buchholz_1_decode(n)), n)
    ...
    def test_explicit_integer_conventions(self) -> None:
        enc = Buchholz1OrdinalEncoder()
        self.assertEqual(enc.positive_int_to_buchholz_1(1), ZERO)
        self.assertEqual(enc.nat_to_buchholz_1(0), ZERO)

        omega = enc.positive_int_to_buchholz_1(3)
        self.assertEqual(omega.to_latex(), "\\omega")
        self.assertEqual(enc.buchholz_1_to_positive_int(omega), 3)
        self.assertEqual(enc.buchholz_1_to_nat(omega), 2)

    def test_invalid_admissible_index_level_raises_value_error(self) -> None:
        enc = Buchholz1OrdinalEncoder()
        with self.assertRaisesRegex(ValueError, "level must be the integer 0 or 1"):
            enc.admissible_rank(2, ZERO)

    def test_argument_code_uses_odd_even_source_index(self) -> None:
        enc = Buchholz1OrdinalEncoder()
        omega = self._term(1, ZERO)

        self.assertEqual(enc.argument_decode(3), omega)
        self.assertEqual(enc.argument_code(omega), 3)
    ...
    def test_omega_renderer_recurses_through_nested_omega_levels(self) -> None:
        ...
        cases = [
            ...
            (self._term(0, self._term(1, self._term(1, self._term(1, ZERO)))), "\\psi(\\Omega^{\\Omega})"),
            (self._term(1, omega), "\\Omega\\omega^{\\omega}"),
            (self._term(1, epsilon_zero), "\\Omega\\psi(\\Omega)"),
