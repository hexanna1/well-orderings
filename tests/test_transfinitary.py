...
class TransfinitaryVeblenMappingTests(unittest.TestCase):
    ...
    def test_nested_transfinitary_position_decodes_by_column_index(self) -> None:
        ...
        ordinal = Ordinal.from_terms([(enc._matrix_from_index(367), 1)])
        self.assertEqual(ordinal, lvo_approximant)
    ...
    def test_initial_prime_index_targets(self) -> None:
        ...
        for index, expected in cases:
            ordinal = Ordinal.from_terms([(enc._matrix_from_index(index), 1)])
    ...
    def test_column_lvo_inverse_examples(self) -> None:
        ...
        cases = [
            (73, self._term((ZERO, self._sum(self._term((ONE, ONE), (ZERO, ONE)), ONE)))),
            (
                7507903,
                self._term(
                    (ONE, omega),
        ...
        for expected_index, expected_ordinal in cases:
            with self.subTest(index=expected_index):
                ordinal = Ordinal.from_terms([(enc._matrix_from_index(expected_index), 1)])
