...
def _cmp_lesser_remainder(
    ...
    for index, (position, value) in enumerate(lesser_entries):
        ...
        if value_cmp > 0:
            return 1
        return 1 if index + 1 < len(lesser_entries) else 0
    return -1
...
def _cmp_matrix(a: Matrix, b: Matrix) -> int:
    ...
    a_index = 0
    b_index = 0
    while a_index < len(a.entries) or b_index < len(b.entries):
        if b_index >= len(b.entries):
            position, a_value = a.entries[a_index]
            b_value = ZERO
            a_index += 1
        elif a_index >= len(a.entries):
            position, b_value = b.entries[b_index]
            a_value = ZERO
            b_index += 1
        else:
            a_position, a_value = a.entries[a_index]
            b_position, b_value = b.entries[b_index]
            position_cmp = _cmp_ordinal(a_position, b_position)
            if position_cmp > 0:
                position = a_position
                b_value = ZERO
                a_index += 1
...
class TransfinitaryVeblenEncoder:
    def __init__(self) -> None:
        ...
        self._ordinal_cache: dict[int, Ordinal] = {}
    ...
    def ordinal(self, n: int) -> Ordinal:
        ...
        cached = self._ordinal_cache.get(n)
        ...
        ordinal = Ordinal._from_normal_form_terms(terms)
        self._ordinal_cache[n] = ordinal
        return ordinal
