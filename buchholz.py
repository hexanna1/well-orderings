...
class Principal:
    ...
    def __hash__(self) -> int:
        value = self._cached_hash
        if value is None:
            value = hash((self.level, self.arg))
...
class Ordinal:
    ...
    def _unchecked(cls, terms: tuple[Term, ...]) -> Ordinal:
        ...
        object.__setattr__(obj, "_cached_hash", None)
        return obj

    def __hash__(self) -> int:
        value = self._cached_hash
        if value is None:
            value = hash((self.terms,))
            object.__setattr__(self, "_cached_hash", value)
        return value
...
def _starts_above_level(ordinal: Ordinal, level: int) -> bool:
    return bool(ordinal.terms) and ordinal.terms[0][0].level > level
...
def _is_epsilon_number(ordinal: Ordinal) -> bool:
    ...
    return coeff == 1 and _starts_above_level(principal.arg, principal.level)
...
def _principal_to_latex(principal: Principal) -> str:
    if principal.level == 0:
        if _is_below_epsilon0(principal.arg):
            return _raw_omega_power_latex(principal.arg)
...
class BuchholzOrdinalEncoder:
    ...
    def prime_table_admissible_raw_codes(self) -> list[int]:
        return list(self._admissible_index(0).admissible_raw_codes)
