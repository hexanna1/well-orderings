...
function encodeArtifact({rawStart, rawCount, rankStart, accepted, bitset}) {
  ...
  const gaps = offsets.map(
    (offset, index) => offset - (index === 0 ? 0 : offsets[index - 1]),
  );
  const bits = bitWidth(maximum(gaps));
  const packed = packValues(gaps, bits);
...
function decodeArtifact(filename) {
  ...
  for (let index = 0; index < offsets.length; index += 1) {
    const offset = offsets[index] + (index === 0 ? 0 : previous);
...
function encodeBundle(maximumRank, segmentSpecs, baseCheckpoints) {
  const segments = segmentSpecs.map((spec, index) => {
    ...
    const baseline = excesses.length === 0
      ? 0
      : excesses.reduce((minimum, excess) => Math.min(minimum, excess));
    for (let index = 0; index < excesses.length; index += 1) {
      excesses[index] -= baseline;
    }
    const bits = bitWidth(maximum(excesses));
    return {...spec, checkpoints, excesses, bits, baseline};
  });
  ...
  for (const segment of segments) {
    ...
    view.setUint32(offset + 16, segment.baseline, true);
